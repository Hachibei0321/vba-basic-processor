' ===============================================
' プロシージャ名：BasicProcessor（基本処理完全版）- 追加要求対応
' 作成者：関西のおばちゃん
' 作成日：2025/06/16
' 修正日：2025/06/18 - 4つの追加処理を実装
' 概要：PDF取得後の基本データ整理処理（完全版）
'       【最初の処理群】
'       1. Table001のB列削除
'       2. Table002の2行目削除（★新規追加★）
...
Sub 基本処理_完全版()
    ' -----------------------------------------------
    ' PDF取得後の基本データ整理処理（完全版・追加要求対応）
    ' -----------------------------------------------
    
    Dim response As VbMsgBoxResult
    
    ' 実行確認
    response = MsgBox("基本処理（完全版）を実行するで?" & vbCrLf & vbCrLf & _
                      "【最初の処理群】" & vbCrLf & _
                      ...
                      "12. シート名変更" & vbCrLf & vbCrLf & _
                      "実行してもええ？", _
                      vbYesNo + vbQuestion, "基本処理完全版")
    ...
    MsgBox "基本処理（完全版）を開始するで♪", vbInformation, "処理開始"
    
    ' 【最初の処理群】順番大事やで♪
    Call Step1_B列削除処理
    Call Step1_5_Table002の2行目削除処理    ' ★新規追加★
    Call Step2_文字列削除処理_最終版
    Call Step3_x大文字変換処理
    Call Step4_カタカナ半角変換処理
    Call Step4_5_F10T変換処理              ' ★新規追加★
    Call Step4_6_S10T変換処理              ' ★新規追加★
    
    ' 【その後の処理群】
    Call Step5_空白行削除処理
    Call Step5_5_ネコアングル変換処理      ' ★新規追加★
    Call Step6_データ統合処理
