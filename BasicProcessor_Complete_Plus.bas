' ===============================================
' プロシージャ名：BasicProcessor（基本処理完全版+プレート処理）
' 作成者：関西のおばちゃん
' 作成日：2025/06/16
' 修正日：2025/06/18 - プレート処理追加（END PL/PL-の平均値計算＋統合）
' 概要：PDF取得後の基本データ整理処理（完全版+プレート処理）
'       【最初の処理群】
'       1. Table001のB列削除
'       2. Table002の2行目削除
...
'       8. A~E列空欄行削除
'       9. ﾈｺｱﾝｸﾞﾙ変換処理
'       10. プレート処理（END PL/PL-統合）（★新規追加★）
'       11. Table002データ統合
'       12. Table002のテーブル形式解除
'       13. Table001 → "原価リスト"に名前変更
...
' ===============================================
' メイン処理：基本処理（完全版+プレート処理）
' ===============================================

Sub 基本処理_完全版プラス()
    ' -----------------------------------------------
    ' PDF取得後の基本データ整理処理（完全版+プレート処理対応）
    ' -----------------------------------------------
    
    Dim response As VbMsgBoxResult
    
    ' 実行確認
    response = MsgBox("基本処理（完全版+プレート処理）を実行するで?" & vbCrLf & vbCrLf & _
                      "【最初の処理群】" & vbCrLf & _
                      ...
                      "10. プレート処理（END PL/PL-統合）" & vbCrLf & _
                      "11. Table002統合" & vbCrLf & _
                      "12. テーブル形式解除" & vbCrLf & _
                      "13. シート名変更" & vbCrLf & vbCrLf & _
                      "実行してもええ？", _
                      vbYesNo + vbQuestion, "基本処理完全版+プレート処理")
    ...
    MsgBox "基本処理（完全版+プレート処理）を開始するで♪", vbInformation, "処理開始"
    ...
    MsgBox "基本処理（完全版+プレート処理）完了♪" & vbCrLf & _
...
Sub Step5_6_プレート処理()
    ' -----------------------------------------------
    ' B列に「END PL」または「PL-」を含み、D列が数値の行を処理
    ' 1. 該当行のD列の平均値を計算
    ' 2. 最上段の行のD列に平均値、B列を「プレート」に変更
    ' 3. 最上段以外の該当行を削除
    ' ※このステップは「原価リスト」シート変更後に実行されるので、
    '   データ統合処理の前に実行するか、処理タイミングを調整が必要
    ' -----------------------------------------------
    ...
    Debug.Print "=== ステップ5.6: プレート処理開始 ==="
    
    ' Table001シートを取得（データ統合前なので、まだ原価リストになってない）
    On Error Resume Next
    Set ws = ThisWorkbook.Worksheets("Table001 (Page 1) ")
    If ws Is Nothing Then Set ws = ThisWorkbook.Worksheets("Table001 (Page 1)")
    On Error GoTo 0
    ...
    With ws
        ...
        For i = 1 To lastRow
            ...
            If (InStr(bValue, "END PL") > 0 Or InStr(bValue, "PL-") > 0) And IsNumeric(dValue) And dValue <> "" Then
...
Sub 結果確認_完全版プラス()
    ...
    With ws
        ...
        msg = "【完全版+プレート処理結果確認】" & vbCrLf & vbCrLf
        ...
        msg = msg & "♪ プレート処理済み（END PL/PL-統合）" & vbCrLf
        msg = msg & "♪ テーブル形式解除済み（エラー修正済み）" & vbCrLf & vbCrLf
        msg = msg & "基本処理（完全版+プレート処理）が完了したで♪"
